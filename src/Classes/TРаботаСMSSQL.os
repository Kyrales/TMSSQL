...
Процедура ИнициироватьПараметры()
	...
	// УчетнаяЗаписьАгента 1С на сервере 1С
	ПараметрыПодключения.Вставить("УчетнаяЗаписьАгента", "");
...
Функция ВосстановитьИзРезервнойКопииБД(МассивИменаФайлов, ТаблицаФайловБДИсточника="") Экспорт
	...
	ТекстЗапросаНаПереводВSingle = "ALTER DATABASE [%1] SET SINGLE_USER WITH ROLLBACK IMMEDIATE
	|";

	ТекстЗапросаНаПереводВSingle = СтрШаблон(ТекстЗапросаНаПереводВSingle, ПараметрыПодключения.ИмяБазыДанныхSQL);

	ТекстЗапроса = ТекстЗапроса + ТекстЗапросаНаПереводВSingle;
...
// Изменяет владельца базы на учетную запись агента сервера 1С из параметров подключения.
//
// Возвращаемое значение:
//   Булево   - Истина - Если запрос выполнен без ошибок. Ложь - Если были ошибки
//
Функция ИзменитьВладельцаПослеВосстановленияБД() Экспорт)
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, 
		ПараметрыПодключения.УчетнаяЗаписьАгента);

	Если ВыполнитьЗапрос(ТекстЗапроса, ПараметрыПодключения.ИмяБазыДанныхSQL) = Истина Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
